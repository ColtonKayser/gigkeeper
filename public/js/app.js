    ...
    this.editGig = function(gig) {
      $http({
        ...
        data: {
          ...
          netPay:
            this.updatedPay -
            this.updatedExtraneousExpenses -
            (this.updatedTravelDistance / this.updatedMilesPerGallon) *
              this.updatedCostOfGas,
          payPerHour:
            (this.updatedPay - this.updatedGasExpense) /
            (this.updatedGigLength + this.updatedTravelTime)
        }
      }).then(
        function(response) {
          controller.getGigs();
